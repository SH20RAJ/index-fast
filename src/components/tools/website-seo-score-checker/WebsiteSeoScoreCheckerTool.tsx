...
export default function WebsiteSeoScoreCheckerTool() {
  ...
  const handleCheck = async () => {
    ...
    try {
      const res = await fetch("/api/tools/check-seo-score", {
        method: "POST",
              ...
              >
                ...
                <div className="flex flex-col md:flex-row items-center gap-8 bg-primary/5 rounded-3xl p-8 border border-primary/10">
                  ...
                  <div className="space-y-1 text-center md:text-left">
                    <h3 className="text-xl font-bold tracking-tight">Overall SEO Health</h3>
                    <p className="text-sm text-muted-foreground max-w-sm">
                      We found <span className="text-primary font-bold">{result.issues.length}</span> potential improvements to boost your search rankings.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground px-1">Detected Issues</h3>
                  <div className="grid gap-3">
                    {result.issues.map((issue, idx) => (
                      <div key={idx} className="p-4 rounded-2xl bg-muted/30 border border-border/50 flex items-start gap-4">
                        <div className={cn(
                          "h-8 w-8 rounded-full flex items-center justify-center shrink-0",
                          issue.type === "error" ? "bg-red-500/10 text-red-600" : issue.type === "warning" ? "bg-amber-500/10 text-amber-600" : "bg-primary/10 text-primary"
                        )}>
                          {issue.type === "error" ? <XCircle className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm font-bold leading-none">{issue.title}</p>
                ...
                <div className="p-6 rounded-[2rem] bg-zinc-950 text-white relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="space-y-2 text-center md:text-left">
                      <h4 className="text-lg font-bold tracking-tight">Automate your SEO audits</h4>
                      <p className="text-sm text-zinc-400 max-w-md">
                        IndexFast Pro scans your entire site daily and uses AI to help you fix SEO issues instantly.
                      </p>
                    </div>
