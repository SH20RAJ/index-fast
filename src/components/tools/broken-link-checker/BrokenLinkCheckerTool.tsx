...
export default function BrokenLinkCheckerTool() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      const initialLinks = data.links.map((u: string) => ({
        url: u,
        status: 0,
        ok: false,
        loading: false,
      ...
      for (let i = 0; i < initialLinks.length; i++) {
        ...
        try {
          const checkRes = await fetch("/api/tools/check-url", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url: initialLinks[i].url }),
          });
  ...
  const stats = {
    total: links.length,
    healthy: links.filter(l => l.status === 200).length,
    broken: links.filter(l => l.status !== 200 && l.status !== 0).length,
    pending: links.filter(l => l.status === 0).length,
  };
        ...
        <CardContent className="p-6 sm:p-8 space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button
                onClick={handleCheck}
                disabled={isExtracting || isChecking || !url.trim()}
              ...
              >
                ...
                {isExtracting ? "Extracting..." : isChecking ? "Checking..." : "Scan Links"}
              </Button>
            </div>
          </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-4 rounded-2xl bg-muted/30 border border-border/50 text-center">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Found</p>
                  ...
                  <div className="p-4 rounded-2xl bg-zinc-500/5 border border-zinc-500/10 text-center">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600/70 mb-1">Checking</p>
                ...
                <div className="p-6 rounded-[2rem] bg-zinc-950 text-white relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="space-y-2 text-center md:text-left">
                      <h4 className="text-lg font-bold tracking-tight">Audit your entire site</h4>
                      <p className="text-sm text-zinc-400 max-w-md">
                        IndexFast Pro scans your whole website for broken links daily and helps you fix them before search engines notice.
                      </p>
                    </div>
