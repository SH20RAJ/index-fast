...
export default function KeywordDensityCheckerTool() {
  ...
  const handleCheck = async () => {
    ...
    try {
      const res = await fetch("/api/tools/check-keyword-density", {
        method: "POST",
            ...
            {result && (
              ...
              >
                ...
                <div className="flex items-center gap-4 px-1">
                  ...
                  <div>
                    <h3 className="text-xl font-bold tracking-tight">{result.totalWords.toLocaleString()}</h3>
                    <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Total Words Found</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground px-1 flex items-center gap-2">
                    <TrendingUp className="h-4 w-4" /> Top Keywords
                  </h3>
                ...
                <div className="p-6 rounded-[2rem] bg-zinc-950 text-white relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="space-y-2 text-center md:text-left">
                      <h4 className="text-lg font-bold tracking-tight">Perform deep keyword research</h4>
                      <p className="text-sm text-zinc-400 max-w-md">
                        IndexFast Pro finds hidden keyword opportunities and tracks your ranking for every page automatically.
                      </p>
                    </div>
