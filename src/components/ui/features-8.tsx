...
export function Features() {
  return (
    <section className="bg-gray-50 py-16 md:py-32 dark:bg-transparent overflow-hidden">
              ...
              <Card className="relative h-full flex overflow-hidden hover:shadow-xl transition-all duration-300 hover:-tranzinc-y-1">
                <CardContent className="relative m-auto size-fit pt-6">
                  <div className="relative flex h-24 w-56 items-center">
                    ...
                    <span className="mx-auto block w-fit text-5xl font-black">
                      120+
                    </span>
                  </div>
                  <h2 className="mt-6 text-center text-2xl font-black tracking-tight text-foreground dark:text-white">
                    Search Engines
                  </h2>
                </CardContent>
              </Card>
            ...
            <motion.div className="col-span-full sm:col-span-3 lg:col-span-2" variants={itemVariants}>
              <Card className="relative h-full overflow-hidden hover:shadow-xl transition-all duration-300 hover:-tranzinc-y-1 group">
                <CardContent className="pt-6">
                  ...
                  >
                    <Shield className="m-auto size-12 text-primary group-hover:scale-110 transition-transform duration-300" strokeWidth={1} />
                  </motion.div>
              ...
              <Card className="relative h-full overflow-hidden hover:shadow-xl transition-all duration-300 hover:-tranzinc-y-1 group">
                <CardContent className="grid pt-6 sm:grid-cols-2">
                  <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                    ...
                    >
                      <Zap className="m-auto size-5 text-amber-500" strokeWidth={1} />
                    </motion.div>
                  ...
                  <div className="rounded-tl-(--radius) relative -mb-6 -mr-6 mt-6 h-fit border-l border-t p-6 py-6 sm:ml-6 bg-muted/30">
                    ...
                    >
                      <motion.path
                        ...
                        fill="url(#paint0_linear_features8_wave)"
                        initial={{ opacity: 0 }}
                        whileInView={{ opacity: 1 }}
                        viewport={{ once: true }}
                      ...
                      <defs>
                        <linearGradient
                          id="paint0_linear_features8_wave"
                          x1="0.85108"
                          y1="0.947876"
                          x2="0.85108"
                          y2="230.114"
            ...
            <motion.div className="col-span-full lg:col-span-3" variants={itemVariants}>
              <Card className="relative h-full overflow-hidden hover:shadow-xl transition-all duration-300 hover:-tranzinc-y-1">
                <CardContent className="grid h-full pt-6 sm:grid-cols-2">
                  ...
                  <div className="before:bg-(--color-border) relative mt-6 before:absolute before:inset-0 before:mx-auto before:w-px sm:-my-6 sm:-mr-6 bg-muted/20">
                    <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                      {[
                        { name: "indexfast.co", img: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=64&h=64&fit=crop&crop=face", align: "end" },
                        { name: "myblog.io", img: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=64&h=64&fit=crop&crop=face", align: "start" },
                        { name: "shopsite.com", img: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=64&h=64&fit=crop&crop=face", align: "end" },
                      ].map((item, idx) => (
                        <motion.div 
                          key={idx}
                          className={cn(
                            "relative flex items-center gap-2",
                            item.align === "end" ? "w-[calc(50%+0.875rem)] justify-end" : "ml-[calc(50%-1rem)]"
                          )}
                          initial={{ x: item.align === "end" ? 20 : -20, opacity: 0 }}
                          whileInView={{ x: 0, opacity: 1 }}
                          viewport={{ once: true }}
                          transition={{ delay: 0.5 + idx * 0.1 }}
                        >
                          {item.align === "start" && (
                             <div className="ring-background size-8 ring-4 shrink-0 overflow-hidden rounded-full">
                               <img className="size-full object-cover" src={item.img} alt="" />
                             </div>
                          )}
                          <span className="block h-fit rounded border bg-background px-2 py-1 text-[10px] font-bold shadow-sm whitespace-nowrap dark:bg-card">
                            {item.name}
                          </span>
                          {item.align === "end" && (
                            <div className="ring-background size-7 ring-4 shrink-0 overflow-hidden rounded-full">
                              <img className="size-full object-cover" src={item.img} alt="" />
                            </div>
                          )}
                        </motion.div>
